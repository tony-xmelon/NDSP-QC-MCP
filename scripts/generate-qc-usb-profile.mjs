...
const subscriptions = profile.liveSubscriptions.join(", ");
const constantName = (name) => name.replace(/([a-z0-9])([A-Z])/g, "$1_$2").toUpperCase();
const javaMessageTypes = Object.entries(profile.messageTypes)
  .map(([name, value]) => `    static final int MESSAGE_TYPE_${constantName(name)} = ${value};`)
  .join("\n");
const rustMessageTypes = Object.entries(profile.messageTypes)
  .map(([name, value]) => `pub const MESSAGE_TYPE_${constantName(name)}: u16 = ${value};`)
  .join("\n");
const pythonMessageTypes = Object.entries(profile.messageTypes)
  .map(([name, value]) => `MESSAGE_TYPE_${constantName(name)} = ${value}`)
  .join("\n");

const java = `// Generated by scripts/generate-qc-usb-profile.mjs. Do not edit by hand.
...
const rust = `// Generated by scripts/generate-qc-usb-profile.mjs. Do not edit by hand.
...
const python = `# Generated by scripts/generate-qc-usb-profile.mjs. Do not edit by hand.
...
for (const [path, content] of outputs) {
  if (process.argv.includes("--check")) {
    ...
    if (current.replaceAll("\r\n", "\n") !== content) {
      console.error(`${path} is not generated from contracts/qc-usb-profile.v1.json`);
      stale = true;
    }
  } else {

...
export const OFFICIAL_BLOCK_VISUALS: Record<OfficialBlockVisualKey, OfficialBlockVisual> = {
  ...
  gate: { key: "gate", tile: [400, 82], color: "#ffd236" }
};
...
export function officialBlockVisual(block: GridBlock): OfficialBlockVisual {
  ...
  if (category.includes("modulation") || /(^|\s)mod(\s|$)/.test(category) || category.includes("chorus") || category.includes("flanger") || category.includes("phaser")) return OFFICIAL_BLOCK_VISUALS.modulation;
